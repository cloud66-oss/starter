...
func init() {
	...
	flag.StringVar(&flagGenerator, "g", "all", "what kind of files need to be generated by starter:\n    	-g all = Dockerfile / service.yml / docker-compose.yml\n    	-g dockerfile = only the Dockerfile\n    	-g service = only the service.yml + Dockerfile (cloud 66 specific)\n    	-g compose = only the docker-compose.yml + Dockerfile\n    	")
}
...
func main() {
	...
	if flagGenerator == "all" || flagGenerator == "service" {
	...
	if flagGenerator == "all" || flagGenerator == "compose" {
